...
def main():
    logger.info("Start Final Model Processing")
    ...
    dataset = np.load(AUDIO_CONSTANTS.RESOURCES_PATH + "//models//data_chroma24_hop512_new_distribution_0905.npy"
                      , allow_pickle=True)
    limit = round(len(dataset) * 0.8)
    # All of the data for training process
    training_dataset = dataset
    # Testing is not important anymore
    testing_dataset = dataset[limit:]
    ...
    input_shape = (24, 87, 1)
    ch_recog_cnn = ChRecogCNN("model_ChRecogCNN_FinalModel", input_shape)
